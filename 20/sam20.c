...
struct elem* find_name(struct elem** q, char *name){
...
void delete_dict(struct elem **q){
    struct elem *p;
    for (int i = 0; i < m; i++){
        p = q[i];
        while (q[i] != NULL){
...
void delete_name(struct elem** q, char *name){
    struct elem* d;
    d = find_name(q, name);
    if (d != NULL){
        for (int i = 0; i < m; i++){
...
int main()
{
    ...
    put_name(name1, strlen(name1), b);
    put_name(name2, strlen(name2), b);
...
/*
Vicktor Aleksey Igor Aleksandr Demyan Vicktor Sanya Petr Ivan Papusha Sergey
Sergey
*/